...
Vex.Flow.StaveNote.prototype.init = function(note_struct) {
  ...
  //jensming add 9-1-2011
  this.size = note_struct.size;

  // Pull note rendering properties from duration.
  this.glyph = Vex.Flow.durationToGlyph(this.duration, this.size);
  if (!this.glyph) {
...
Vex.Flow.StaveNote.prototype.setRenderOptions = function () {
	this.size = this.size.replace('_w', '');
	Vex.Merge(this.render_options, Vex.Flow.stavenote_render_codes[this.size]);
	return this;
}
...
//jensming add size adjustment to LEFT and RIGHT positions based on size parameter
Vex.Flow.StaveNote.prototype.getModifierStartXY = function(position, index) {
  ...
  if (position == Vex.Flow.Modifier.Position.LEFT) {
    //x = -1 * 2;  // extra_left_px	
	x = -1 * Vex.Flow.shift_factor[this.size];  // extra_left_px
  } else if (position == Vex.Flow.Modifier.Position.RIGHT) {
    //x = this.glyph.head_width + this.x_shift + 2; // extra_right_px	
	x = this.glyph.head_width + this.x_shift + Vex.Flow.shift_factor[this.size]; // extra_right_px
  }
...
Vex.Flow.StaveNote.prototype.preFormat = function() {
  ...
  //jensming change on 9-2-2011 - to add size factor for accidental formatting
  if (this.modifierContext) {
	this.size = this.size.replace('_w', '');
	this.modifierContext.accidental_spacing = Vex.Flow.accidental_spacing[this.size];
