 ...
 */
Vex.Flow.Beam = function(notes, size) {
  if (arguments.length > 0) this.init(notes, size);
}
 ...
 */
Vex.Flow.Beam.prototype.init = function(notes, size) {
  ...
  this.notes = notes;
  this.beam_count =
    Vex.Flow.durationToGlyph(this.notes[0].getDuration(), this.size).beam_count;
  this.render_options = {
    beam_width: 5,
    max_slope: 0.25,
    min_slope: -0.25,
...
//jensming add 9-3-2011
Vex.Flow.Beam.prototype.setBeamWidth = function () {
	this.render_options.beam_width = Vex.Flow.beam_width[this.size];
}
