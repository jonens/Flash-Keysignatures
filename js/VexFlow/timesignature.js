 ...
 * @constructor 
 */
Vex.Flow.TimeSignature = function (timeSpec, size) { 
  if (arguments.length > 0) this.init(timeSpec, size); 
}
...
Vex.Flow.TimeSignature.prototype.init = function(timeSpec, size) {
  Vex.Flow.TimeSignature.superclass.init();

  //jensming add 9-2-2011
  this.size = size;
  //-------------------
  this.setPadding(15);
  this.point = 40;
...
Vex.Flow.TimeSignature.prototype.parseTimeSpec = function(timeSpec) {
  if (timeSpec == "C" || timeSpec == "C|") {
	...
	glyphInfo.point = Vex.Flow.clef_points[this.size];
...
Vex.Flow.TimeSignature.prototype.makeTimeSignatureGlyph = function(topNums, botNums) {
  this.point = Vex.Flow.clef_points[this.size];
